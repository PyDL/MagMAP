...
# limit the size of drms queries
query_chunk = datetime.timedelta(days=30)
...
# avoid querying more than a month at a time
while period_end >= period_start:
    if (period_end - period_start) > query_chunk:
        chunk_end = period_start + query_chunk
    else:
        chunk_end = period_end
    ...
    target_times = pd.date_range(start=rounded_start, end=chunk_end, freq=interval_cadence).to_pydatetime()
    ...
    # iterate to next download chunk
    period_start += query_chunk
