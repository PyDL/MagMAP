...
def set_assim_wghts(br_map, assim_method="mu4_upton"):
    ...
    if assim_method == "mu":
        ...
        # update layer order
        br_map.layers = pd.concat([br_map.layers, pd.DataFrame(dict(layer_num=[1, 2], layer_name=["assim_weight", "mu"],
        ...
        br_map.assim_weight[br_map.data == br_map.no_data_val] = 0
        # update layer order
        br_map.layers = pd.concat([br_map.layers, pd.DataFrame(dict(layer_num=[1, ], layer_name=["assim_weight", ],
        ...
        # update layer order
        br_map.layers = pd.concat([br_map.layers, pd.DataFrame(dict(layer_num=[2, ], layer_name=["uncertainty", ],
                                                               py_field_name=['uncert', ]))])

        # pass mu as an additional layer (already exists in br_map) by adding it to the layer order
        br_map.layers = pd.concat([br_map.layers, pd.DataFrame(dict(layer_num=[3, ], layer_name=["mu", ],
                                                               py_field_name=['mu', ]))])
    else:
